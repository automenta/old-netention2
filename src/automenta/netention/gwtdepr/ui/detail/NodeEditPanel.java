...
import automenta.netention.value.node.NodeNotEquals;
...
abstract public class NodeEditPanel extends DockPanel {
	...
	public NodeEditPanel(DetailData pd) {
		...
		//		ScrollPanel propertyListScroll = new ScrollPanel(propertyList);
		//		propertyListScroll.setAlwaysShowScrollBars(true);
		//		propertyList.setHeight("100%");
	...
	abstract protected void onDeleteNode();
	abstract protected void onSaveNode();
	...
	public void setNode(DetailData pd) {
		...
		synchronized (pd.getProperties()) {
			for (PropertyValue v : pd.getProperties()) {
				...
				if (pp instanceof PropertyPanel)
					propertyPanels.add((PropertyPanel) pp);
				propertyList.add(pp);							
	...
	protected Widget newPropertyPanel(final PropertyValue v) {
		...
		//Note: this if/else is ordered to handle sub-types of Value's prior to their super-types

			 if (v instanceof RealIs) 			{		w = new RealPropertyPanel(prop, (RealIs)v);		}
		else if (v instanceof RealEquals) 		{		w = new RealPropertyPanel(prop, (RealEquals)v);		}
		
		else if (v instanceof StringIs)			{		w = new StringPropertyPanel(prop, (StringIs)v);		}
		else if (v instanceof StringEquals)		{		w = new StringPropertyPanel(prop, (StringEquals)v);		}
		
		else if (v instanceof IntegerIs)		{		w = new IntPropertyPanel(prop, (IntegerIs)v);		}
		else if (v instanceof IntegerEquals)	{		w = new IntPropertyPanel(prop, (IntegerEquals)v);		}
		
		else if (v instanceof NodeIs) 			{		w = new NodePropertyPanel(prop, (NodeIs)v); }
		else if (v instanceof NodeNotEquals) 	{		w = new NodePropertyPanel(prop, (NodeNotEquals)v); }
		else if (v instanceof NodeEquals) 		{		w = new NodePropertyPanel(prop, (NodeEquals)v); }
