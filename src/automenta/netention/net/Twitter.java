...
public class Twitter extends twitter4j.Twitter {
    ...
    protected void updateMentions(final PlexusGraph pg, Status s, final Message m) {
...
//            }).start();
    ...
    public void addStatus(PlexusGraph pg, Status s) {
        Message m = new Message(s.getText(), null);

        pg.graph.addVertex(m);

        Agent ac = new Agent(s.getUser().getName(), s.getUser().getProfileImageURL());
    ...
    public void getPublicTimeline(PlexusGraph pg) throws Exception {
